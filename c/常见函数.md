...
//  std::int64_t n = *reinterpret_cast<std::int64_t*>(&d); // aliasing violation
    ...
    // 是的，buf是一个静态空间变量，它在定义时在栈上分配了一块内存，大小为 sizeof(s)。它用于在该内存块上对类型 s 进行就地（in-place）构造。由于 s 是一个 pod 类型，因此可以使用 new (buf) s 的语法来构造对象。
    ...
    //没有基类和虚函数
    //没有私有或保护的非静态数据成员
    //pod类型在c++中有很多用途，比如对于一些需要直接操作内存的代码，使用pod类型可以更方便地进行内存管理和数据处理。例如，在某些情况下，可能需要将一个对象以字节序列的形式写入文件或网络，并在另一个进程或机器上读取它。这时，使用pod类型可以确保数据的二进制兼容性，从而避免出现问题。
